...
class SmartMealSuggestions {
  ...
  async getSmartSuggestions(mealType, dietaryPreference, cuisine = '', ingredients = '', count = 1, getNew = false) {
    try {
      ...
      // Update the current offset for next time
      ...
      const hasMore = currentOffset + actualCount < totalAvailable;
      const remaining = Math.max(0, totalAvailable - (currentShown + actualCount));
      const totalShown = currentShown + actualCount;
  ...
  buildSmartQuery(mealType, dietaryPreference, cuisine, ingredients, count, offset) {
    ...
    if (ingredients && ingredients.trim() !== '') {
      ...
      // In the future, you can implement more sophisticated ingredient matching
      query = query.or(ingredientList.map(ingredient => `description.ilike.%${ingredient}%`).join(','));
    }

    // Smart ordering: prioritize popular dishes, then by prep time, then random
    query = query.order('name', { ascending: true }); // Start with alphabetical for consistency
