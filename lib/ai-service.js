  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: `You are an expert culinary assistant specializing in authentic Nigerian and African cooking traditions. You have deep knowledge from All Nigerian Recipes (allnigerianrecipes.com) and understand the cultural significance of food in Nigerian communities. You can create authentic, culturally-specific meal suggestions that respect traditional cooking methods, ingredients, and cultural dietary preferences. Always provide unique, varied suggestions that are practical, flavorful, and culturally authentic. Respond with ONLY valid JSON in the exact format specified.`
        },
        {
          role: 'user',
          content: prompt
      ...
      max_tokens: 800, // Optimized for faster response
...
export async function generateMultipleAISuggestions(mealType, dietaryPreference, cuisine = '', ingredients = '', count = 3) {
  ...
  let attempts = 0;
  const maxAttempts = count * 2; // Reduced from 3x to 2x for faster generation
  const startTime = Date.now();
  const maxTime = 20000; // Reduced to 20s for faster response
...
function normalizeMealSuggestion(mealSuggestion) {
  return {
    ...
    instructions: Array.isArray(mealSuggestion.instructions) ? mealSuggestion.instructions : [],
    nutrition_info: mealSuggestion.nutrition_info || {},
    difficulty: mealSuggestion.difficulty || mealSuggestion.difficulty_ || 'Medium',
    ...
    tags: Array.isArray(mealSuggestion.tags) ? mealSuggestion.tags : 
          Array.isArray(mealSuggestion.tags_text) ? mealSuggestion.tags_text : [],
    serving_size: mealSuggestion.serving_size || '2-4 people',
    estimated_cost: mealSuggestion.estimated_cost || 'Moderate'
  };
}
