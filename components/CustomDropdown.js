...
export default function CustomDropdown({ options, value, onChange, placeholder = 'Select...', label }) {
  ...
  useEffect(() => {
    if (open && buttonRef.current) {
      ...
      const dropdownHeight = Math.min(options.length * 60 + 20, 300); // Approximate height
  ...
  return (
    <div className="w-full relative" ref={containerRef}>
      ...
      {/* Dropdown */}
      {open && (
        <div className="fixed z-50 left-4 right-4 md:left-auto md:right-auto md:w-full">
          <div 
            className={`bg-white/95 backdrop-blur-xl border-2 border-gray-100 rounded-2xl shadow-2xl py-2 max-h-64 overflow-y-auto ${
              position === 'top' ? 'bottom-full mb-2' : 'top-full mt-2'
            }`}
            style={{
              top: position === 'top' ? 'auto' : buttonRef.current?.getBoundingClientRect().bottom + 8,
              bottom: position === 'top' ? window.innerHeight - buttonRef.current?.getBoundingClientRect().top + 8 : 'auto',
              left: '0',
              right: '0',
              maxHeight: '300px'
            }}
